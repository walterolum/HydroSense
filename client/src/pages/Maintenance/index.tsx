  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard loading={loading} title="Total Requests"     value={requests.length} icon={Wrench}        color="blue"   />
        <StatCard loading={loading} title="Pending / Assigned"
        ...
        <StatCard loading={loading} title="Overdue"            value={stats?.overdue || 0} subtitle="> 7 days unresolved" icon={AlertTriangle} color="red"   />
        <StatCard loading={loading} title="Avg Resolution"     value={`${stats?.avg_resolution_hours || 0}h`} subtitle="Hours to complete" icon={CheckCircle} color="green" />
      </div>

      {/* Charts */}
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="card">
          <h3 className="section-title mb-3">By Status</h3>
        ...
        <div className="card">
          <h3 className="section-title mb-3">By Priority</h3>
          <ResponsiveContainer width="100%" height={180}>
            <PieChart>
        ...
        <div className="card">
          <h3 className="section-title mb-3"><DollarSign size={16} className="text-green-600" /> Maintenance Funds</h3>
          {funds?.summary ? (
            <div className="space-y-3">
              {[
                { label: 'Total Balance',   value: funds.summary.total_balance   || 0, color: 'text-green-700',  bg: 'bg-green-50'  },
                { label: 'Total Collected', value: funds.summary.total_collected || 0, color: 'text-blue-700',   bg: 'bg-blue-50'   },
                { label: 'Total Spent',     value: funds.summary.total_spent     || 0, color: 'text-orange-700', bg: 'bg-orange-50' },
              ].map(f => (
            ...
            <p className="text-sm text-gray-400 text-center py-6">No fund data</p>
          )}
        </div>
      </div>
      ...
      {stats?.spare_parts?.length > 0 && (
        <div className="card">
          <h3 className="section-title mb-3">Spare Parts Inventory</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {stats.spare_parts.map((p: any) => (
              <div key={p.id} className={`p-3 rounded-lg border ${p.quantity <= p.min_quantity ? 'border-red-300 bg-red-50' : 'border-gray-100 bg-gray-50'}`}>
                ...
                {p.quantity <= p.min_quantity && <div className="text-xs text-red-600 mt-1">⚠ Low stock</div>}
      ...
      <div className="card p-0">
        <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-800 flex flex-wrap items-center gap-3">
          ...
          <button onClick={openModal} className="btn-primary text-xs ml-auto"><Plus size={14} /> New Request</button>
        </div>
        <div className="table-container">
          <table className="table">
            <thead><tr>
              <th className="th">#</th><th className="th">Water Point</th><th className="th">District</th>
              <th className="th">Issue</th><th className="th">Priority</th><th className="th">Status</th>
              <th className="th">Technician</th><th className="th">Cost Est.</th>
              <th className="th">Created</th><th className="th">Action</th>
            </tr></thead>
            <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-100 dark:divide-gray-800">
              {requests.map(r => (
                <tr key={r.id} className="tr">
                  ...
                  <td className="td text-xs">{r.technician_name || <span className="text-gray-400">Unassigned</span>}</td>
                  ...
                  <td className="td">
                    {r.status === 'pending'     && <button onClick={() => handleStatusUpdate(r.id, 'in_progress')} className="text-xs text-blue-600 hover:underline">Start</button>}
                    {r.status === 'in_progress' && <button onClick={() => handleStatusUpdate(r.id, 'completed')}  className="text-xs text-green-600 hover:underline">Complete</button>}
                    {r.status === 'completed'   && <span className="text-xs text-green-500">✓ Done</span>}
                  </td>
                </tr>
              ))}
