...
router.post('/send', requireRole('national_admin','district_officer'), (req, res) => {
  ...
  const result = db.prepare(`INSERT INTO notification_log (recipient_type, recipient_id, recipient_contact, channel, subject, message, status, reference_type, reference_id) VALUES (?,?,?,?,?,?,'sent',?,?)`).run(
