...
router.get('/stats', (req, res) => {
  ...
  const total = db.prepare(`SELECT COUNT(*) as count FROM water_quality_tests wqt JOIN water_points wp ON wqt.water_point_id = wp.id WHERE 1=1 ${distFilter}`).get(...params);
  const safe = db.prepare(`SELECT COUNT(*) as count FROM water_quality_tests wqt JOIN water_points wp ON wqt.water_point_id = wp.id WHERE wqt.overall_safe = 1 ${distFilter}`).get(...params);
  const avgScore = db.prepare(`SELECT AVG(wqt.water_safety_score) as avg FROM water_quality_tests wqt JOIN water_points wp ON wqt.water_point_id = wp.id WHERE 1=1 ${distFilter}`).get(...params);
  const contaminated = db.prepare(`SELECT wp.district, wp.name, wqt.e_coli_cfu, wqt.turbidity_ntu, wqt.ph, wqt.tested_at, wqt.water_safety_score FROM water_quality_tests wqt JOIN water_points wp ON wqt.water_point_id = wp.id WHERE wqt.overall_safe = 0 ${distFilter} ORDER BY wqt.tested_at DESC LIMIT 20`).all(...params);
  const byDistrict = db.prepare(`SELECT wp.district, COUNT(*) as tests, SUM(CASE WHEN wqt.overall_safe=1 THEN 1 ELSE 0 END) as safe_count, AVG(wqt.water_safety_score) as avg_score FROM water_quality_tests wqt JOIN water_points wp ON wqt.water_point_id = wp.id GROUP BY wp.district ORDER BY avg_score ASC`).all();
...
router.post('/tests', (req, res) => {
  ...
  if (!safe) {
    db.prepare(`INSERT INTO alerts (alert_type, severity, water_point_id, district, title, message, source)
