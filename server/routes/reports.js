...
router.get('/stats', (req, res) => {
  const db = getDb();
  const total = db.prepare('SELECT COUNT(*) as count FROM community_reports').get();
  const byStatus = db.prepare('SELECT status, COUNT(*) as count FROM community_reports GROUP BY status').all();
  const byChannel = db.prepare('SELECT channel, COUNT(*) as count FROM community_reports GROUP BY channel').all();
  const byIssue = db.prepare('SELECT issue_type, COUNT(*) as count FROM community_reports GROUP BY issue_type ORDER BY count DESC').all();
  const byDistrict = db.prepare('SELECT district, COUNT(*) as count FROM community_reports GROUP BY district ORDER BY count DESC').all();
  const openCritical = db.prepare("SELECT COUNT(*) as count FROM community_reports WHERE status = 'open' AND severity = 'high'").get();
...
router.put('/:id', (req, res) => {
  ...
  db.prepare("UPDATE community_reports SET status=COALESCE(?,status), assigned_to=COALESCE(?,assigned_to), resolution_notes=COALESCE(?,resolution_notes), updated_at=datetime('now') WHERE id=?").run(status, assigned_to, resolution_notes, req.params.id);
