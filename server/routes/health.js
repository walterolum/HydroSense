...
router.get('/stats', (req, res) => {
  const db = getDb();
  const total_cases = db.prepare('SELECT SUM(cases) as total, SUM(deaths) as deaths, SUM(hospitalizations) as hosp FROM health_incidents').get();
  const active_outbreaks = db.prepare("SELECT COUNT(*) as count FROM health_incidents WHERE outbreak_status IN ('outbreak','alert')").get();
  const water_linked = db.prepare('SELECT COUNT(*) as count FROM health_incidents WHERE water_source_linked = 1').get();
  const by_disease = db.prepare('SELECT disease_type, SUM(cases) as total_cases, SUM(deaths) as total_deaths, COUNT(*) as incidents FROM health_incidents GROUP BY disease_type ORDER BY total_cases DESC').all();
  const by_district = db.prepare('SELECT district, SUM(cases) as total_cases, SUM(deaths) as total_deaths, COUNT(*) as incidents FROM health_incidents GROUP BY district ORDER BY total_cases DESC').all();
  const by_status = db.prepare('SELECT outbreak_status, COUNT(*) as count FROM health_incidents GROUP BY outbreak_status').all();
  res.json({
    success: true,
    data: {
...
router.post('/incidents', (req, res) => {
  ...
  if ((cases || 0) >= 10) {
    db.prepare(`INSERT INTO alerts (alert_type, severity, district, title, message, source) VALUES ('health', ?, ?, ?, ?, 'health_system')`).run(
...
router.put('/incidents/:id', (req, res) => {
  ...
  db.prepare('UPDATE health_incidents SET cases=COALESCE(?,cases), deaths=COALESCE(?,deaths), outbreak_status=COALESCE(?,outbreak_status), investigation_notes=COALESCE(?,investigation_notes), resolved_date=COALESCE(?,resolved_date) WHERE id=?').run(cases, deaths, outbreak_status, investigation_notes, resolved_date, req.params.id);
