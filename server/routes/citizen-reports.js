...
router.post('/', authMiddleware, async (req, res) => {
  ...
  db.prepare(`INSERT INTO citizen_report_tracking (report_id, status, note, updated_by) VALUES (?, 'submitted', 'Report submitted successfully', ?)`).run(reportId, req.user.id);
  ...
  if (lang !== 'en' || original_text) {
    postToAI('/ai/incident-analysis/enhanced-analyze', {
      ...
      if (analysis?.analysis) {
        ...
        try {
          db.prepare(`INSERT INTO incident_analysis (report_id, ai_severity, ai_category, ai_urgency, ai_risk_score, extracted_location, ai_summary, is_duplicate, confidence_score, speech_to_text, response_recommendation, analyzed_at) VALUES (?,?,?,?,?,?,?,0,?,?,?,datetime('now'))`).run(
...
router.get('/', authMiddleware, (req, res) => {
  ...
  const reports = data.map(r => {
    const media = db.prepare(`SELECT id, media_type, mime_type FROM report_media WHERE report_id = ?`).all(r.id);
    const tracking = db.prepare(`SELECT * FROM citizen_report_tracking WHERE report_id = ? ORDER BY created_at DESC`).all(r.id);
...
router.get('/stats', authMiddleware, (req, res) => {
  ...
  const total = db.prepare(`SELECT COUNT(*) as c FROM citizen_reports${where}`).get(...params).c;
  const byStatus = db.prepare(`SELECT status, COUNT(*) as c FROM citizen_reports${where ? where + ' GROUP BY status' : ' GROUP BY status'}`).all(...params);
  const byType = db.prepare(`SELECT incident_type, COUNT(*) as c FROM citizen_reports${where ? where + ' GROUP BY incident_type' : ' GROUP BY incident_type'}`).all(...params);
  const byChannel = db.prepare(`SELECT channel, COUNT(*) as c FROM citizen_reports${where ? where + ' GROUP BY channel' : ' GROUP BY channel'}`).all(...params);
  const pending = db.prepare(`SELECT COUNT(*) as c FROM citizen_reports${where ? where + " AND status = 'pending'" : " WHERE status = 'pending'"}`).get(...params).c;
  const resolved = db.prepare(`SELECT COUNT(*) as c FROM citizen_reports${where ? where + " AND status = 'resolved'" : " WHERE status = 'resolved'"}`).get(...params).c;
  res.json({ success: true, data: { total, by_status: byStatus, by_type: byType, by_channel: byChannel, pending, resolved } });
});

router.get('/:id', authMiddleware, (req, res) => {
  const db = getDb();
  const report = db.prepare(`SELECT cr.*, COALESCE(ia.ai_severity, cr.severity) as ai_severity, ia.ai_risk_score, ia.ai_category, ia.is_duplicate, ia.duplicate_of_id, ia.is_false_report, ia.confidence_score, ia.speech_to_text, ia.image_analysis, ia.response_recommendation, ia.analyzed_at FROM citizen_reports cr LEFT JOIN incident_analysis ia ON cr.id = ia.report_id WHERE cr.id = ?`).get(req.params.id);
  ...
  const tracking = db.prepare(`SELECT ct.*, u.name as updated_by_name FROM citizen_report_tracking ct LEFT JOIN users u ON ct.updated_by = u.id WHERE ct.report_id = ? ORDER BY ct.created_at DESC`).all(report.id);
  const assignments = db.prepare(`SELECT * FROM task_assignments WHERE report_id = ?`).all(report.id);

  res.json({ success: true, data: { ...report, media, tracking, assignments } });
});

router.put('/:id/status', authMiddleware, requireRole('national_admin','district_officer','health_officer'), (req, res) => {
  const db = getDb();
  ...
  db.prepare(`UPDATE citizen_reports SET status = ?, updated_at = datetime('now') WHERE id = ?`).run(status, req.params.id);
  db.prepare(`INSERT INTO citizen_report_tracking (report_id, status, note, updated_by) VALUES (?, ?, ?, ?)`).run(req.params.id, status, note || null, req.user.id);

  res.json({ success: true, message: 'Status updated' });
});

router.post('/:id/analyze', authMiddleware, requireRole('national_admin','district_officer'), (req, res) => {
  const db = getDb();
  ...
  const existing = db.prepare(`SELECT id FROM incident_analysis WHERE report_id = ?`).get(report.id);
  if (existing) {
    db.prepare(`UPDATE incident_analysis SET ai_severity=?, ai_category=?, ai_risk_score=?, ai_summary=?, response_recommendation=?, analyzed_at=datetime('now') WHERE report_id=?`).run(
    ...
    db.prepare(`INSERT INTO incident_analysis (report_id, ai_severity, ai_category, ai_urgency, ai_risk_score, ai_summary, response_recommendation) VALUES (?,?,?,?,?,?,?)`).run(
  ...
  const updated = db.prepare(`SELECT * FROM incident_analysis WHERE report_id = ?`).get(report.id);
