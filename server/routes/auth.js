...
router.post('/send-otp', (req, res) => {
  ...
  db.prepare(`DELETE FROM otp_codes WHERE email = ? AND used = 0`).run(email.toLowerCase().trim());
  db.prepare(`INSERT INTO otp_codes (email, otp, purpose, expires_at) VALUES (?, ?, ?, ?)`).run(email.toLowerCase().trim(), otp, purpose, expires);
...
router.post('/verify-otp', (req, res) => {
  ...
  const record = db.prepare(`SELECT * FROM otp_codes WHERE email = ? AND otp = ? AND used = 0 AND expires_at > datetime('now') ORDER BY created_at DESC LIMIT 1`).get(email.toLowerCase().trim(), otp);
  ...
  if (user) {
    db.prepare(`UPDATE users SET otp_verified = 1 WHERE id = ?`).run(user.id);
...
router.post('/resend-otp', (req, res) => {
  ...
  db.prepare(`DELETE FROM otp_codes WHERE email = ? AND used = 0`).run(email.toLowerCase().trim());
  db.prepare(`INSERT INTO otp_codes (email, otp, purpose, expires_at) VALUES (?, ?, 'registration', ?)`).run(email.toLowerCase().trim(), otp, expires);
...
router.get('/me', authMiddleware, (req, res) => {
  const db = getDb();
  const user = db.prepare('SELECT id, name, email, role, district, sub_county, phone, organization, avatar, active, last_login, created_at FROM users WHERE id = ?').get(req.user.id);
...
router.put('/profile', authMiddleware, (req, res) => {
  ...
  const updated = db.prepare('SELECT id, name, email, role, district, sub_county, phone, organization, avatar, active, last_login, created_at FROM users WHERE id = ?').get(req.user.id);
...
router.get('/users', authMiddleware, (req, res) => {
  const db = getDb();
  const users = db.prepare('SELECT id, name, email, role, district, sub_county, organization, phone, avatar, active, last_login, created_at FROM users ORDER BY name').all();
  res.json({ success: true, data: users });
});

/* ── Create user (admin only) ── */
router.post('/users', authMiddleware, requireRole('national_admin'), (req, res) => {
  const db = getDb();
...
router.put('/users/:id', authMiddleware, requireRole('national_admin'), (req, res) => {
  const db = getDb();
  ...
  if (email) {
    const emailConflict = db.prepare('SELECT id FROM users WHERE email = ? AND id != ?').get(email.toLowerCase().trim(), userId);
...
router.put('/users/:id/password', authMiddleware, requireRole('national_admin'), (req, res) => {
...
router.delete('/users/:id', authMiddleware, requireRole('national_admin'), (req, res) => {
  const userId = parseInt(req.params.id);
...
router.post('/forgot-password', (req, res) => {
  ...
  db.prepare(`DELETE FROM otp_codes WHERE email = ? AND purpose = 'password_reset'`).run(email.toLowerCase().trim());
  db.prepare(`INSERT INTO otp_codes (email, otp, purpose, expires_at) VALUES (?, ?, 'password_reset', ?)`).run(email.toLowerCase().trim(), otp, expires);
...
router.post('/reset-password', (req, res) => {
  ...
  const db = getDb();
  const record = db.prepare(`SELECT * FROM otp_codes WHERE email = ? AND otp = ? AND purpose = 'password_reset' AND used = 0 AND expires_at > datetime('now') ORDER BY created_at DESC LIMIT 1`).get(email.toLowerCase().trim(), otp);
  if (!record) return res.status(400).json({ success: false, error: 'Invalid or expired OTP' });
...
router.get('/language', authMiddleware, (req, res) => {
  const row = getDb().prepare('SELECT language FROM users WHERE id = ?').get(req.user.id);
