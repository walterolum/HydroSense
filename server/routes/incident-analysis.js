...
router.get('/dashboard', (req, res) => {
  const db = getDb();
  const total = db.prepare(`SELECT COUNT(*) as c FROM incident_analysis`).get().c;
  const highRisk = db.prepare(`SELECT COUNT(*) as c FROM incident_analysis WHERE ai_risk_score >= 70`).get().c;
  const duplicates = db.prepare(`SELECT SUM(is_duplicate) as c FROM incident_analysis`).get().c || 0;
  const falseReports = db.prepare(`SELECT SUM(is_false_report) as c FROM incident_analysis`).get().c || 0;
  const byCategory = db.prepare(`SELECT ai_category, COUNT(*) as c FROM incident_analysis WHERE ai_category IS NOT NULL GROUP BY ai_category ORDER BY c DESC`).all();
  const avgConfidence = db.prepare(`SELECT AVG(confidence_score) as avg FROM incident_analysis`).get().avg || 0;
  const recentAnalysis = db.prepare(`SELECT ia.*, cr.incident_type, cr.district, cr.description, cr.created_at as report_date FROM incident_analysis ia JOIN citizen_reports cr ON ia.report_id = cr.id ORDER BY ia.analyzed_at DESC LIMIT 10`).all();
  const pendingAnalysis = db.prepare(`SELECT COUNT(*) as c FROM citizen_reports cr LEFT JOIN incident_analysis ia ON cr.id = ia.report_id WHERE ia.id IS NULL`).get().c;
...
router.get('/pending', (req, res) => {
  const db = getDb();
  const pending = db.prepare(`SELECT cr.* FROM citizen_reports cr LEFT JOIN incident_analysis ia ON cr.id = ia.report_id WHERE ia.id IS NULL ORDER BY cr.created_at ASC LIMIT 20`).all();
  res.json({ success: true, data: pending });
});

router.post('/analyze-all-pending', (req, res) => {
  const db = getDb();
  const pending = db.prepare(`SELECT cr.* FROM citizen_reports cr LEFT JOIN incident_analysis ia ON cr.id = ia.report_id WHERE ia.id IS NULL ORDER BY cr.created_at ASC LIMIT 20`).all();
  let analyzed = 0;
  ...
  for (const report of pending) {
    ...
    db.prepare(`INSERT INTO incident_analysis (report_id, ai_severity, ai_category, ai_urgency, ai_risk_score, is_duplicate, is_false_report, confidence_score, ai_summary, response_recommendation) VALUES (?,?,?,?,?,?,?,?,?,?)`).run(
      report.id,
